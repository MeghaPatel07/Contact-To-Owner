...
const Customer_QR = () => {
  ...
  const {Contact,  Name,Email,Contact1,Contact2,State,City,Area,Pin,Address,Package, IsActive,srNo,byAdmin,pdf,InviceNo } = values;
  ...
  const handleCheckByAdmin = (e) => {
    setValues({ ...values, byAdmin: e.target.checked });
  };
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        if(response.isOk)
          {
             axios.put(
              `${process.env.REACT_APP_API_URL_CONTACTUS}/api/auth/create/pdf/${response.data._id}`,response,
            // Send necessary data to server (values object)
          );
          }
...
form.append("byAdmin", values.byAdmin);
form.append("InviceNo", values.InviceNo);
form.append("srNo", values.srNo);
    if (Object.keys(errors).length === 0) {
      await axios
        .put(
  ...
  const [errContact1, setErrContact1] = useState(false);
  const [errContact2, setErrContact2] = useState(false);
...
const [errPackage, setErrPackage] = useState(false);
  ...
  const validClassContact =
    errContact && isSubmit ? "form-control is-invalid" : "form-control";
const validClassContact1 =
  errContact1 && isSubmit ? "form-control is-invalid" : "form-control";
const validClassContact2 =
  errContact2 && isSubmit ? "form-control is-invalid" : "form-control";
...
const validClassPin =
  errPin && isSubmit ? "form-control is-invalid" : "form-control";
const validClassAddress =
  errAddress && isSubmit ? "form-control is-invalid" : "form-control";
const validClassPackage =
  errPackage && isSubmit ? "p-0 h-100 form-control is-invalid" : "p-0 h-100 form-control";

// Add similar validClass variables for other fields as needed
  ...
  const validate = (values) => {
    ...
    if (values.Contact1 === "") {
      ...
      setErrContact1(true);
    } else if (!/^\d{10}$/.test(values.Contact1)) {
      errors.Contact1 = "Invalid contact number!";
      setErrContact1(true);
    } else {
      setErrContact1(false);
    }


    if (values.Contact2 === "") {
      errors.Contact2 = "Contact 2 is required!";
      setErrContact2(true);
    } else if (!/^\d{10}$/.test(values.Contact1)) {
      errors.Contact2 = "Invalid contact number!";
      setErrContact2(true);
    } else {
      setErrContact2(false);
    }
...
<div className="form-floating mb-3">
  ...
  {isSubmit && <p className="text-danger">{formErrors.Contact}</p>}
</div>
<div className="form-floating mb-3">
  <Input
    type="text"
    className={validClassContact1}
  ...
  />
  <Label>
    Contact1 for QR <span className="text-danger">*</span>
  </Label>
...
<div className="form-floating mb-3">
  <Input
    type="text"
    className={validClassContact2}
    placeholder="Enter Contact2"
    name="Contact2"
    value={values.Contact2}
    onChange={handleChange}
  />
  <Label>Contact2 for QR<span className="text-danger">*</span></Label>
  {isSubmit && <p className="text-danger">{formErrors.Contact2}</p>}
</div>
...
<div className="form-check mb-2">
              <Input
                ...
                name="By Admin"
                value={byAdmin}
                onChange={handleCheckByAdmin}
              />
              <Label className="form-check-label">By Admin</Label>
            </div>
            <div className="form-check mb-2">
              <Input
                type="checkbox"
...
<div className="form-floating mb-3">
  ...
  />
  <Label>
  QR  Contact1 <span className="text-danger">*</span>
  </Label>
  {isSubmit && <p className="text-danger">{formErrors.Contact1}</p>}
</div>
...
<div className="form-floating mb-3">
  <Input
    ...
    onChange={handleChange}
  />
  <Label>QR Contact2<span className="text-danger">*</span></Label>
  {isSubmit && <p className="text-danger">{formErrors.Contact2}</p>}
